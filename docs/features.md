...
Runtime events are recorded to tapes (default under `~/.eli/tapes`).
Context is reconstructed from tape records, not accumulated in session state.

## Builtin Batteries

- **CLI**: `run`, `chat`, `gateway`, `login`, `hooks` via Typer.
- **Model runtime**: agent loop with tool use, backed by [Republic](https://github.com/eliagent/republic).
...
External plugins are loaded via Python entry points (`group="eli"`).
