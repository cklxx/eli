---
title: "Baby Eli: From Inspiration to Bootstrap Milestone"
date: 2025-07-16
description: "How Eli draws from modern agent design, and why fixing a single mypy issue is a meaningful step toward self-improving AI."
---

# Baby Eli: From Inspiration to Bootstrap Milestone

## Genesis: Inspiration from Modern Agents

Eli is a CLI-first AI agent, built to "Eli it. Build it." The project draws direct inspiration from [How to Build an Agent](https://ampcode.com/how-to-build-an-agent) and [Tiny Agents: Building LLM-Powered Agents from Scratch](https://huggingface.co/blog/tiny-agents). Both resources distill the essence of tool-using, loop-based, composable, and extensible agents.

But Eli is also a response to the new wave of self-improving, self-hosting agents: think Claude Code, SWE-agent, and the broader "self-bootstrapping" movement. The goal: an agent that can not only help you build, but can help build (and fix) itself.

## Architecture: ReAct Loop, Tools, and CLI

### The ReAct Loop

At the heart of Eli is a classic ReAct loop, implemented in [`src/eli/agent/core.py`](https://github.com/PsiACE/eli/blob/19c015/src/eli/agent/core.py):
...
Tools are registered via a `ToolRegistry` ([`src/eli/agent/tools.py`](https://github.com/psiace/eli/blob/19c015/src/eli/agent/tools.py)), and each tool is a Pydantic model with validation and metadata. For example, the `RunCommandTool` blocks dangerous commands and validates input:
...
The CLI ([`src/eli/cli/app.py`](https://github.com/psiace/eli/blob/19c015/src/eli/cli/app.py)) is built with Typer and Rich, providing a modern, user-friendly interface. The renderer ([`src/eli/cli/render.py`](https://github.com/psiace/eli/blob/19c015/src/eli/cli/render.py)) supports debug toggling, minimal/verbose TAAO (Thought/Action/Action Input/Observation) output, and clear error reporting.
...
Eli aspires to self-improvement. The first tangible milestone? Fixing the very first mypy error: adding a missing return type annotation to `Renderer.__init__`, check out the [commit](https://github.com/PsiACE/eli/commit/87cdcc).
...
## Looking Forward: Eli as a Bootstrap Agent

Eli is still early. But the architecture is in place for:
...
- [Project on GitHub](https://github.com/psiace/eli)
