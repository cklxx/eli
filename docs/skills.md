...
Eli treats skills as discoverable `SKILL.md` documents with validated frontmatter.
...
Validation rules from `src/eli/skills.py`:
...
Legacy path note: `.agent/skills` is still scanned for compatibility, but Eli emits a deprecation warning and prefers `.agents/skills`.
