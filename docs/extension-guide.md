...
This guide explains how to implement Eli hooks with `@hookimpl`, and how those implementations are executed in the current runtime.

## 1) Import And Basic Shape

Use the marker exported by Eli:

```python
from eli import hookimpl
```
...
```toml
[project.entry-points."eli"]
my_plugin = "my_package.plugin:my_plugin"
```

`EliFramework.load_hooks()` loads builtin first, then entry points in `group="eli"`.
...
If that import is missing, the tool module never runs, nothing is inserted into `eli.tools.REGISTRY`, and the tool will not be available to the agent or CLI completion.
...
At runtime, Eli discovers builtin skills from `<site-packages>/skills`, so packaged skills in that location are loaded automatically.
...
- `provide_channels`: called by `call_many_sync` in `EliFramework.get_channels()`
- `system_prompt`, `provide_tape_store`: consumed by `EliFramework` and the builtin `Agent`
...
- Using stale hook names: always confirm against `src/eli/hookspecs.py`.
